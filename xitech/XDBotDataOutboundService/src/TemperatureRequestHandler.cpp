...
TemperatureRequestHandler::TemperatureRequestHandler(Poco::OSP::BundleContext::Ptr pContext)
	: _pContext(pContext)
{
	_pPrefs = Poco::OSP::ServiceFinder::find<Poco::OSP::PreferencesService>(_pContext);

	std::string deviceName("io.macchina.serialport#3");
...
TemperatureRequestHandler::~TemperatureRequestHandler()
{

}

Poco::OSP::BundleContext::Ptr TemperatureRequestHandler::context() const
{
	return _pContext;
}

void TemperatureRequestHandler::handleRequest(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response)
{
	if (request.getMethod() == "HEAD" || request.getMethod() == "GET" || request.getMethod() == "POST") {
		...
		if (_pSerialDevice) {
			...
			memcpy(&temperature, vbuf, 2);
			memcpy(&humidity, vbuf+2, 2);

			Poco::JSON::Object messsage;
			messsage.set("temperature", (float)temperature/10.0f);
			messsage.set("humidity", (float)humidity/10.0f);
...
Poco::Net::HTTPRequestHandler* TemperatureRequestHandlerFactory::createRequestHandler(const Poco::Net::HTTPServerRequest& request)
{
	std::cout << "request.getURI(): " << request.getURI() << std::endl;
	return new TemperatureRequestHandler(context());
}
