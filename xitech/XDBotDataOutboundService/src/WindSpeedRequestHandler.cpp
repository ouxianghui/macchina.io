...
WindSpeedRequestHandler::WindSpeedRequestHandler(Poco::OSP::BundleContext::Ptr pContext)
	: _pContext(pContext)
{
	_pPrefs = Poco::OSP::ServiceFinder::find<Poco::OSP::PreferencesService>(_pContext);

	std::string deviceName("io.macchina.serialport#4");
...
WindSpeedRequestHandler::~WindSpeedRequestHandler()
{

}

Poco::OSP::BundleContext::Ptr WindSpeedRequestHandler::context() const
{
	return _pContext;
}

void WindSpeedRequestHandler::handleRequest(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response)
{
...
Poco::Net::HTTPRequestHandler* WindSpeedRequestHandlerFactory::createRequestHandler(const Poco::Net::HTTPServerRequest& request)
{
	std::cout << "request.getURI(): " << request.getURI() << std::endl;
	return new WindSpeedRequestHandler(context());
}
